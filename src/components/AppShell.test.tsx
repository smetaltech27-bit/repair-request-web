...
describe('AppShell notifications', () => {
  ...
  it('does not render removed mobile-only navigation items', async () => {
    ...
    expect(screen.getByRole('navigation')).toHaveClass('lg:space-y-1', 'lg:overflow-y-hidden')
    expect(screen.getByRole('link', { name: 'หน้าแรก' })).toHaveClass('lg:py-2.5', 'lg:text-base')
